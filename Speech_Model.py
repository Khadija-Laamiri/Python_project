...
class SpeechModel:
  ...
  def __init__(self, input_loader: charge_entree, input_size: int, num_classes: int):
  ...
  def add_training_ops(self, learning_rate: bool = 1e-3, learning_rate_decay_factor: float = 0, max_gradient_norm: float = 5.0, momentum: float = 0.9):
    ...
    if self.labels is not None:
      with tf.name_scope('training'):
        ...
        self.update = optimizer.apply_gradients(zip(clipped_gradients, trainables), global_step=self.global_step, name='apply_gradients')
  ...
  def add_decoding_ops(self, language_model: str = None, lm_weight: float = 0.8, word_count_weight: float = 0.0,
    ...
    language model : we are not using any langugage model but we can use models like kenlm if needed and use 
    ...
    with tf.name_scope('decoding'):
      ...
      self.valid_word_count_weight = tf.placeholder_with_default(valid_word_count_weight, shape=(), name='valid_word_count_weight')
      ...
      """
      self.decoded, self.log_probabilities = tf.nn.ctc_greedy_decoder(self.logits, self.sequence_lengths // 2, merge_repeated=True)
  ...
  def _convolution(self, value, filter_width, stride, input_channels, out_channels, apply_non_linearity=True):
    ...
    with tf.variable_scope('convolution_layer_{}'.format(layer_id)) as layer:
      ...
      filters = tf.get_variable('filters', shape=[filter_width, input_channels, out_channels], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
...
class Wav2LetterModel(SpeechModel):
  ...
  def _create_network(self, num_classes):
    ...
    # 1 layer with high kernel width and output size [batch size, max time/2, 2000]
...
def create_default_model(command, input_size: int, speech_input: charge_entree) -> SpeechModel:
  model = Wav2LetterModel(input_loader=speech_input, input_size=input_size, num_classes=29)
  
  if command == 'train':
    model.add_training_ops(learning_rate=tf.flags.learning_rate, learning_rate_decay_factor=tf.flags.learning_rate_decay_factor, max_gradient_norm=tf.flags.max_gradient_norm, momentum=tf.flags.momentum)
    model.add_decoding_ops()
  
  else:
    model.add_training_ops()
    model.add_decoding_ops(language_model=None, lm_weight=0.8, word_count_weight=0.0, valid_word_count_weight=2.3)

  model.finalize(log_dir='log', run_name='best-weights', run_type='train')
