...
class Test(ExecuteurDeTests):

    def CreerGenerateurDEchantillons(self, limit_count: int):
        return self.lecteur.load_samples('test', loop_infinitely=False, limit_count=limit_count, feature_type='power')

    def ObtenirNmbrLimiteChargeur(self):
        return 0
    ...
    def run(self):
        ...
        with tf.Session() as sess:
            model = self.CreerModele(sess)
            coord = self.DémarrerPipeline(sess) 
