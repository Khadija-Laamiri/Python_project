...
def calculatePowerSpectrogram(audio_data, samplerate, n_mels=128, n_fft=512, hop_length=160):
  spect = librosa.feature.melspectrogram(y=audio_data, sr=samplerate, n_mels=n_mels, n_fft=n_fft, hop_length=hop_length)
...
class DatasetReader:
  ...
  def _transform_sample(cls, audio_file, preprocess_fnc):
    ...
    audio_fragments = preprocess_fnc(audio_data=audio_data, samplerate=samplerate)
  ...
  def generate_samples(self, directory, preprocess_fnc):
    audio_files = list(recursiveTraverse(self._data_directory, '*.flac'))
...
class Preprocess:
  ...
  def run(self):
    reader = DatasetReader("D:\\train-clean-100\\LibriSpeech\\train-clean-100")
