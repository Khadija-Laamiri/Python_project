...
#Une méthode privée qui prend une liste d'entrées 
# sous forme de tableaux numpy et les  transforme en un tenseur numpy 
...
class charger_entre_unique(charge_entree):

  """
  Alimente manuellement les entrées uniques à l'aide du dictionnaire de flux
  """

  def __init__(self, input_size):
    #super pour apperler le const de class parent
  ...
  def get_inputs(self):
    # retourne tensors pour inputs and sequence lengths

    return self.inputs, self.sequence_lengths, None

  def get_feed_dict(self):
    """
    renvoie le dictionnaire de flux pour la prochaine étape du modèle
    """

    if self.speech_input is None:
      raise ValueError("L'entrée vocale doit d'abord être fournie en utilisant set_input !")
...
class charge_plusieur_entre(charge_entree):
